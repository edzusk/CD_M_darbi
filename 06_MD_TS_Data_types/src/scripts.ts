...
const someTitles = getTitles(products)
...
const somePrice = formatPrice(products[0])
...
const someCat: Cat = {name: 'Mr. Fluff', kind: 'British Shorthair', age: 4}

type SoftDrink = {
  title:string
  price:number
}
const anyProduct: SoftDrink = {title: 'Dr. Pepper', price: 4.5}
const productTitle:string = anyProduct.title
const priceWithTaxes:number = anyProduct.price * (1.25)
const upperCaseTitle:string = anyProduct.title.toUpperCase()
...
const currentAccount = fetchAccount(4) as {id: number, name: string, currency: string}/* add Type Assertion here */
